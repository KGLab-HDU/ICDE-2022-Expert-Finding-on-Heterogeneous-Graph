...
public class FindAllNB {
	...
	public FindAllNB(int graph[][], int vertexType[], int edgeType[]) {
		this.graph = graph;
	...
	private Map<Integer, Set<Integer>> buildGraph() {
		...
		for(int curId = 0;curId < graph.length;curId ++) {
			if(vertexType[curId] == queryMPath.vertex[0]) {
				...
				findPNeighbors(curId, curId, 0, visitList, pnbMap);//find all the pnbs of curId
	...
	private void findPNeighbors(int startID, int curId, int index, List<Set<Integer>> visitList, Map<Integer, Set<Integer>> pnbMap) {
		int targetVType = queryMPath.vertex[index + 1], targetEType = queryMPath.edge[index];
		
		int nbArr[] = graph[curId];
		Set<Integer> pnbSet = pnbMap.get(startID);
		for(int i = 0;i < nbArr.length;i += 2) {
			...
			if(!visitSet.contains(nbVertexID) && targetVType == vertexType[nbVertexID] && targetEType == edgeType[nbEdgeID]) {
				if(index + 1 < queryMPath.pathLen) {
					findPNeighbors(startID, nbVertexID, index + 1, visitList, pnbMap);
